    ...
    <div class="animated fadeIn">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <form action="{{ route('melaksanakan-tugas.store') }}" method="post">
                            @csrf
                            ...
                            is-invalid @enderror">
                                    ...
                                    <option value="{{ $item->id }}"
                                        {{ old('pegawaitgs_id') == $item->id ? 'selected' : null }}>
                            ...
                            is-invalid @enderror">
                                    ...
                                    @foreach ($pegawais as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('pegawaitgs_id') == $item->id ? 'selected' : null }}>{{ $item->nip}}
                                    </option>
                                    @endforeach
                                </select>
                                @error('pegawaitgs_id')
                            ...
                            is-invalid @enderror" value="{{ old('nama_tgs') }}">
                                @error('nama_tgs')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
