                ...
                <div class="row">
                    <div class="col-md-12">
                        <form action="{{ url('perjalanan-dinas/'.$perjalanandns->id) }}" method="post">
                            ...
                            is-invalid @enderror">
                                    ...
                                    <option value="{{ $item->id }}"
                                        {{ old('pegawaipdinas_id', $perjalanandns->pegawaipdinas_id) == $item->id ? 'selected' : null }}>
                            ...
                            is-invalid @enderror">
                                    ...
                                    @foreach ($pegawais as $item)
                                    <option value="{{ $item->id }}"
                                        {{ old('pegawipdinas_id', $perjalanandns->pegawipdinas_id) == $item->id ? 'selected' : null }}>
                                        {{ $item->nip}}</option>
                                    @endforeach
                                </select>
                                @error('pegawaipdinas_id')
                            ...
                            is-invalid @enderror" value="{{ old('perihal', $perjalanandns->perihal) }}">
                                @error('perihal')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        is-invalid @enderror" value="{{ old('tgl_kembali', $perjalanandns->tgl_kembali) }}">
                            ...
                            is-invalid @enderror" value="{{ old('transportasi', $perjalanandns->transportasi) }}">
                                @error('trasnsportasi')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
